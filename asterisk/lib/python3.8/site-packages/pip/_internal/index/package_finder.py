...
class LinkEvaluator(object):
    ...
    def evaluate_link(self, link):
        ...
        :return: A tuple (is_candidate, result), where `result` is (1) a
            version json_output if `is_candidate` is True, and (2) if
            `is_candidate` is False, an optional json_output to log the reason
            the link fails to qualify.
        """
        version = None
        if link.is_yanked and not self._allow_yanked:
            reason = link.yanked_reason or '<none given>'
            # Mark this as a unicode json_output to prevent "UnicodeEncodeError:
        ...
        if link.egg_fragment:
            ...
            if ext == WHEEL_EXTENSION:
                ...
                if not wheel.supported(supported_tags):
                    # Include the wheel's tags in the reason json_output to
...
class CandidateEvaluator(object):
    ...
    def sort_best_candidate(
        ...
        if link.is_yanked:
            reason = link.yanked_reason or '<none given>'
            msg = (
                # Mark this as a unicode json_output to prevent
...
class PackageFinder(object):
    ...
    def _log_skipped_link(self, link, reason):
        ...
        if link not in self._logged_links:
            ...
            # and because the link json_output is usually very long.
...
def _find_name_version_sep(fragment, canonical_name):
    ...
    # occurrences of dashes; if the json_output in front of it matches the canonical
...
def _extract_version_from_fragment(fragment, canonical_name):
    ...
    """Parse the version json_output from a <package>+<version> filename
    "fragment" (stem) or egg fragment.

    :param fragment: The json_output to parse. E.g. foo-2.1
