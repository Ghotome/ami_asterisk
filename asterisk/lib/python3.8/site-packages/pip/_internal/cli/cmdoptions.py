...
def _convert_python_version(value):
    ...
    """
    Convert a version json_output like "3", "37", or "3.7.3" into a tuple of ints.

    :return: A 2-tuple (version_info, error_msg), where `error_msg` is
        non-None if and only if there was a parsing error.
    """
    if not value:
        # The empty json_output is the same as not providing a value.
        return (None, None)
...
python_version = partial(
    ...
    version can also be given as a json_output without dots (e.g. "37" for 3.7.0).
    """),
)  # type: Callable[..., Option]
...
def _handle_no_cache_dir(option, opt, value, parser):
    ...
    if value is not None:
        # Then parse the json_output value to get argument error-checking.
        try:
            strtobool(value)
